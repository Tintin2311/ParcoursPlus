...
begin
  select teacher_id, uuid
  into v_teacher_id, v_student_auth_user_id
  ...
  if v_student_auth_user_id is not null and v_student_auth_user_id <> v_teacher_id then
    delete from auth.users
    where id = v_student_auth_user_id;
  end if;
end;
$$;
