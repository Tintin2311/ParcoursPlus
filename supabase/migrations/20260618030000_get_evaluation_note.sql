...
begin
  ...
  if not coalesce(v_mode_evaluation, false) or v_bareme_id is null then
    return query select null::numeric, null::numeric, null::uuid, 'no_evaluation_bareme'::text;
    return;
  end if;
  ...
  if not found then
    return query select null::numeric, null::numeric, v_bareme_id, 'no_stats'::text;
    return;
  end if;
  ...
  if coalesce(v_stats.parcours_termine, false) = false
    and coalesce(v_stats.chronometre_finished, false) = false then
    return query select null::numeric, v_max_points, v_bareme_id, 'not_finished'::text;
    return;
  end if;
  ...
  if not found then
    ...
    return query select null::numeric, v_max_points, v_bareme_id, 'no_matching_row'::text;
    return;
  end if;
  ...
  if not found then
    ...
    return query select null::numeric, v_max_points, v_bareme_id, 'no_matching_column'::text;
    return;
  end if;
  ...
  return query select v_note, v_max_points, v_bareme_id, case when v_note is null then 'no_matching_cell' else 'ok' end;
end;
$$;
