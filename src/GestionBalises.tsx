...
const C_FROZEN_BORDER = "rgba(59,130,246,0.78)";
...
const VIEW_MODE_OPTIONS: {
...
}[] = [
  {
    id: "numero",
    title: "Numéro seul",
    subtitle: "Voir uniquement les numéros des balises",
  },
  {
    id: "resume",
    title: "Numéro + formats",
    subtitle: "Voir le numéro et les symboles des formats existants",
  },
  {
    id: "code",
    title: "Numéro + code simple",
    subtitle: "Masquer les balises sans code simple",
  },
  {
    id: "tableau",
    title: "Numéro + tableau",
    subtitle: "Masquer les balises sans tableau",
  },
  {
    id: "poincon",
    title: "Numéro + poinçon",
    subtitle: "Masquer les balises sans poinçon",
  },
  {
    id: "qrcode",
    title: "Numéro + QR code",
    subtitle: "Masquer les balises sans QR code",
  },
];
...
const extractTokensFromAny = (value: any): string[] => {
  ...
  if (typeof value === "object") {
    ...
    if (Array.isArray((value as any).balises)) {
      out.push(...extractTokensFromAny((value as any).balises));
    }
    if (Array.isArray((value as any).balise_ids)) {
      out.push(...extractTokensFromAny((value as any).balise_ids));
    }
  ...
  if (
    ...
    try {
      ...
      return extractTokensFromAny(parsed);
    } catch {
      //
    }
  }
  ...
  for (let r = 0; r < size; r++) {
    ...
    for (let c = 0; c < size; c++) {
      ...
      if (inTopLeft || inTopRight || inBottomLeft) {
        ...
        const border =
          localR === 0 || localR === 4 || localC === 0 || localC === 4;
        const center =
          localR >= 1 && localR <= 3 && localC >= 1 && localC <= 3;
          ...
          <View key={`qr-r-${r}`} style={styles.miniQrRow}>
            {row.map((filled, c) => (
              <View
                key={`qr-${r}-${c}`}
                style={[
                  big ? styles.miniQrPixelBig : styles.miniQrPixel,
                  filled && styles.miniQrPixelDark,
                ]}
              />
            ))}
          </View>
            ...
            {Array.from({ length: previewCols }).map((__, c) => {
              ...
              return (
                <View
                  key={key}
                  style={[
                    styles.miniGridPreviewCell,
                    big && styles.miniGridPreviewCellBig,
                  ]}
                >
                  {!!value ? (
                    <View style={[styles.miniTableInk, big && styles.miniTableInkBig]} />
                  ) : null}
                </View>
              );
            })}
          ...
          <View key={`p-r-${r}`} style={styles.miniGridPreviewRow}>
            {Array.from({ length: previewCols }).map((__, c) => {
              ...
              return (
                ...
                >
                  {active ? (
                    <View style={[styles.miniPunchDot, big && styles.miniPunchDotBig]} />
                  ) : null}
                </View>
              );
            })}
...
const fetchBalisesFromSupabase = async (userId: string): Promise<Balise[]> => {
  ...
  if (error) {
    console.error("❌ fetchBalisesFromSupabase:", error);
    throw error;
  }
...
const fetchBaliseFormatsFromSupabase = async (
  userId: string
): Promise<Map<string, BaliseFormat[]>> => {
...
const fetchParcoursUsageFromSupabase = async (
  allBalises: Balise[],
  userId: string
): Promise<Map<string, ParcoursRef[]>> => {
...
const buildPdfRows = (items: Balise[], usageMap: Map<string, ParcoursRef[]>) =>
  items.map((b) => [
    ...
    String(b.code || "—"),
    String((usageMap.get(b.id) || []).length),
    b.frozen ? "Oui" : "Non",
  ]);
...
const buildCompactHtmlForNativePdf = (
  balises: Balise[],
  usageMap: Map<string, ParcoursRef[]>
) => {
  const sorted = [...balises].sort(
    (a, b) =>
      parseInt(a.numero_balise || "0", 10) -
...
const GestionBalises: React.FC<Props> = ({ setPage = () => {} }) => {
  ...
  const [parcoursUsageMap, setParcoursUsageMap] = useState<Map<string, ParcoursRef[]>>(
    new Map()
  );
  ...
  const [rangeFilterType, setRangeFilterType] = useState<"none" | "range" | "list">(
    "none"
  );
    ...
    try {
      ...
      try {
        ...
        if (raw) {
          ...
          setBalisesState(Array.isArray(cached) ? cached : []);
        }
      } catch {
        //
      }
  ...
  const filteredBalises = useMemo(() => {
    return sortedBalises.filter((balise) => {
      ...
      if (searchLower) {
        if (searchType === "code") {
          matchesSearch = String(balise.code || "")
            .toLowerCase()
            .includes(searchLower);
        } else {
          matchesSearch = String(numero).includes(searchLower);
        }
      }
      ...
      if (filterInactiveChecked && !isInactive) {
        matchesStateFilter = false;
      }
      ...
      if (rangeFilterType === "range" && (rangeStart || rangeEnd)) {
        ...
        matchesRangeFilter =
          (Number.isNaN(s) || numero >= s) && (Number.isNaN(e) || numero <= e);
      ...
      if (!matchesSearch || !matchesStateFilter || !matchesRangeFilter) {
        return false;
      }

      const formats = formatsMap.get(balise.id) || [];
      const hasCode =
        !!String(balise.code || "").trim() || formats.some((f) => f.format_type === "code");
    ...
    async (balise: Balise) => {
      try {
        await AsyncStorage.setItem(
          ...
          JSON.stringify({
            balise_id: balise.id,
            balise_numero: balise.numero_balise,
          })
        );
      } catch {
        //
  ...
  const handleExportPdf = useCallback(async () => {
    try {
      ...
      if (Platform.OS === "web") {
        await exportPdfWeb(sorted, parcoursUsageMap);
      } else {
        await exportPdfNative(sorted, parcoursUsageMap);
      }
    } catch (e: any) {
      ...
      <View style={styles.header}>
        <View style={styles.headerRow}>
          <View style={styles.headerTitleWrap}>
            <Text
              style={[styles.headerTitle, isVerySmall && styles.headerTitleSmall]}
          ...
          {showFilters && (
            <View style={styles.filtersBox}>
              ...
              <View style={[styles.row, styles.searchRowResponsive]}>
                ...
                <TouchableOpacity
                  onPress={() =>
                    setSearchType((t) => (t === "code" ? "number" : "code"))
                  }
                  style={[styles.selectBtn, isVerySmall && styles.selectBtnMobile]}
                  activeOpacity={0.85}
                >
                  <Text style={styles.selectBtnText}>
                    {searchType === "code" ? "Code de balise" : "Numéro de balise"}
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={[styles.row, styles.checkboxWrap]}>
                <TouchableOpacity
                  onPress={() => setFilterFrozenChecked((v) => !v)}
                ...
                >
                  <View
                    style={[
                      styles.checkbox,
                      filterFrozenChecked && styles.checkboxChecked,
                    ]}
                  />
                ...
                <TouchableOpacity
                  onPress={() => setFilterInactiveChecked((v) => !v)}
                  style={styles.checkboxRow}
                  activeOpacity={0.85}
                >
                  <View
                    style={[
                      styles.checkbox,
                      filterInactiveChecked && styles.checkboxChecked,
                    ]}
                  />
              ...
              <View style={[styles.row, { marginBottom: 6, flexWrap: "wrap" }]}>
                <TouchableOpacity
                  ...
                  style={[
                    styles.toggleChip,
                    rangeFilterType === "none" && styles.toggleChipActive,
                  ]}
                  activeOpacity={0.85}
                >
                ...
                <TouchableOpacity
                  onPress={() => {
                    setRangeFilterType("range");
                    setListFilter("");
                  }}
                  style={[
                    styles.toggleChip,
                    rangeFilterType === "range" && styles.toggleChipActive,
                  ]}
                  activeOpacity={0.85}
                >
                ...
                <TouchableOpacity
                  onPress={() => {
                    setRangeFilterType("list");
                    setRangeStart("");
                    setRangeEnd("");
                  }}
                  style={[
                    styles.toggleChip,
                    rangeFilterType === "list" && styles.toggleChipActive,
                  ]}
                  activeOpacity={0.85}
                >
            ...
            <>
              <View style={[styles.grid, { gap }]}>
                {filteredBalises.map((b) => {
                  ...
                  const hasCode =
                    !!String(b.code || "").trim() || formats.some((f) => f.format_type === "code");
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        viewMode === "numero" && styles.tileNumeroOnly,
                        {
                          width: tileSize,
                          height: tileSize,
                          backgroundColor: isUsedInParcours
                            ? C_TILE_USED_BG
                            : C_TILE_UNUSED_BG,
                          borderColor: b.frozen
                            ? C_FROZEN_BORDER
                            : isUsedInParcours
                              ? C_TILE_USED_BORDER
                              : C_TILE_UNUSED_BORDER,
                        },
                      ]}
                      activeOpacity={0.9}
                    ...
                    >
                      ...
                      {viewMode === "numero" ? (
                        <View style={styles.numeroOnlyCenter}>
                          <Text style={[styles.numeroOnlyText, width < 420 && styles.numeroOnlyTextSmall]}>
                            {b.numero_balise || "—"}
                          </Text>
                        </View>
                      ) : viewMode === "resume" ? (
                        <View style={styles.tileContentMulti}>
                          <View style={styles.tileMiniRowLarge}>
                            {hasCode ? (
                              <View style={styles.largeIconChip}>
                            ...
                            {tableauFormat ? (
                              <View style={styles.largeIconChip}>
                                <Table2 size={18} color="#1f2937" />
                              </View>
                            ) : null}

                            {poinconFormat ? (
                              <View style={styles.largeIconChip}>
                                <PunchSymbol size={18} color="#1f2937" />
                              </View>
                            ) : null}

                            {qrFormat ? (
                              <View style={styles.largeIconChip}>
                                <QrCode size={18} color="#1f2937" />
                              </View>
                            ) : null}
                          </View>
                        </View>
                        ...
                        <View style={styles.singlePreviewWrapBig}>
                          {tableauFormat ? (
                            <MiniTablePreview payload={tableauFormat.payload || {}} big />
                          ) : null}
                        </View>
                      ) : viewMode === "poincon" ? (
                        <View style={styles.singlePreviewWrapBig}>
                          {poinconFormat ? (
                            <MiniPoinconPreview payload={poinconFormat.payload || {}} big />
                          ) : null}
                        </View>
                      ) : viewMode === "qrcode" ? (
                        <View style={styles.singlePreviewWrapBig}>
                          {qrFormat ? (
                            <MiniQrPreview value={String(qrFormat.payload?.value ?? "")} big />
                          ) : null}
                        </View>
                      ) : null}
                      ...
                      {b.frozen && (
                        <View style={styles.frozenDot}>
                          <Snowflake size={14} color="#1e3a8a" />
                        </View>
                      )}
                    </TouchableOpacity>
                  );
              ...
              {loaded && filteredBalises.length === 0 && (
                <View style={styles.emptyBox}>
                  ...
                  <Text style={styles.emptyText}>
                    Modifie tes filtres ou le mode d’affichage.
                  </Text>
                </View>
              )}
            </>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      active && styles.viewOptionRowActive,
                    ]}
                  >
